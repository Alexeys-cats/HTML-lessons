...
function prepareSearchData() {
  ...
  htmlData.forEach((item) => {
    allData.push({
      ...
      searchText:
        `${item.name} ${item.description} ${item.example}`.toLowerCase(),
    });
  });
  ...
  Object.keys(cssData).forEach((property) => {
    ...
    allData.push({
      ...
      moz: item.moz,
      searchText:
        `${property} ${item.initial} ${item.values ? item.values.join(' ') : ''} css`.toLowerCase(),
    });
  });
...
function filterByBrowserSupport(results) {
  if (
    !filtersState.browserSupport.webkit &&
  ...
  return results.filter((item) => {
    ...
    if (filtersState.browserSupport.webkit && item.webkit && !item.moz)
      return true;
    if (filtersState.browserSupport.moz && item.moz && !item.webkit)
      return true;
    if (filtersState.browserSupport.both && item.webkit && item.moz)
      return true;
